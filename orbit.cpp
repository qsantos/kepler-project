...
int orbit_orientate(Orbit* o, double longitude_of_ascending_node, double inclination, double argument_of_periapsis, double epoch, double mean_anomaly_at_epoch) {
    ...
    o->orientation = glm::eulerAngleZXZ(longitude_of_ascending_node, inclination, argument_of_periapsis);
    return 0;
...
int orbit_from_state(Orbit* o, CelestialBody* primary, glm::dvec3 position, glm::dvec3 velocity, double epoch) {
    ...
    glm::dvec3 z_axis{0., 0., 1.};

    glm::dvec3 orbital_plane_normal_vector = glm::cross(position, velocity);

    // eccentricity
    double pos_factor = speed * speed / mu - 1 / distance;  // v^2/mu - 1/r
    double vel_factor = glm::dot(position, velocity) / mu;  // r.v / mu
    glm::dvec3 eccentricity_vector = pos_factor * position - vel_factor * velocity;
    double eccentricity = glm::length(eccentricity_vector);

    // periapsis
    // from r(t) = 1. / mu * h / (1. + e cos t)
    double specific_angular_momentum = glm::length(orbital_plane_normal_vector);
    ...
    // required to use glm::angle
    orbital_plane_normal_vector = glm::normalize(orbital_plane_normal_vector);

    // inclination
    double inclination = glm::angle(glm::normalize(orbital_plane_normal_vector), glm::normalize(z_axis));

    // direction of the ascending node
    glm::dvec3 ascend_node_dir;
    if (inclination == 0. || inclination == M_PI) {
        ascend_node_dir = {1., 0., 0.};
    } else {
        ascend_node_dir = glm::cross(z_axis, orbital_plane_normal_vector);
    }

    // longitude of ascending node
    double longitude_of_ascending_node = glm::angle(glm::normalize(x_axis), glm::normalize(ascend_node_dir));
    if (orbital_plane_normal_vector[0] < 0.) {
        longitude_of_ascending_node = - longitude_of_ascending_node;
    }

    // argument of periapsis
    glm::dvec3 periapsis_dir = eccentricity != 0. ? glm::normalize(eccentricity_vector) : x_axis;
    double argument_of_periapsis = glm::orientedAngle(glm::normalize(ascend_node_dir), glm::normalize(periapsis_dir), glm::normalize(orbital_plane_normal_vector));

    // mean anomaly at epoch
    double true_anomaly_at_epoch = glm::orientedAngle(glm::normalize(periapsis_dir), glm::normalize(position), glm::normalize(orbital_plane_normal_vector));
...
static glm::dvec3 _position_from_distance_true_anomaly(Orbit* o, double distance, double true_anomaly) {
    double c = cos(true_anomaly);
    double s = sin(true_anomaly);
    return o->orientation * glm::dvec3{distance*c, distance*s, 0.};
}

static glm::dvec3 _velocity_from_distance_true_anomaly(Orbit* o, double distance, double true_anomaly) {
    double c = cos(true_anomaly);
    double s = sin(true_anomaly);
    double e = o->eccentricity;
    ...
    glm::dvec3 velocity_direction{-distance*s + x*c, distance*c + x*s, 0.};

    double speed = orbit_speed_at_distance(o, distance);

    glm::dvec3 velocity = velocity_direction * (speed / glm::length(velocity_direction));
